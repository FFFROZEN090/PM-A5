...
/**
 * 商品订单服务 Commodity order service
 */
...
public class OrderService {
	...
	/**
	 * 向订单添加项目 Add items to the order
	 * @param order
	 * @param good
	 * @return
	 */
	public Orders addOrderItem(Orders order, Goods good) {
		...
		// 如果购物车已有此项目, 数量+1 (If the shopping cart already has this item, add one to the quantity)
		...
		// 如果当前购物车没有此项目, 创建新条目 (If the current shopping cart does not have this item, create a new item)
		if (notThis) {
			itemList.add(addItem(good));
		}
	...
	/**
	 * 从订单中减少项目 Reduce items from the order
	 * @param order
	 * @param good
	 * @return
	 */
	public Orders lessenIndentItem(Orders order, Goods good) {
		List<Items> itemList = order.getItemList();
		itemList = itemList==null ? new ArrayList<Items>() : itemList;
		// 如果购物车已有此项目, 数量-1 (If the shopping cart already has this item, delete one from the quantity)
		...
		// 如果当前购物车没有项目, 直接返回 (If there is no item in the current shopping cart, return directly)
		if (noneThis) {
			return order;
		}
		order.setTotal(order.getTotal() - good.getPrice());
	...
	/**
	 * 从订单中删除项目 Remove item from order
	 * @param order
	 * @param good
	 * @return
	 */
	public Orders deleteIndentItem(Orders order, Goods good) {
		List<Items> itemList = order.getItemList();
		itemList = itemList==null ? new ArrayList<Items>() : itemList;
		// 如果购物车已有此项目, 数量清零 (If the shopping cart already has this item, the quantity is cleared)
		boolean noneThis = true;
		int itemAmount = 0;
		...
		// 如果已经没有项目, 返回null (If there are no items, return null)
	...
	public void pay(Orders order) {
		...
		// 微信或支付宝支付时, 模拟支付完成 (When paying with WeChat or Alipay, the simulated payment is completed)
	...
	/**
	 * 获取某用户全部订单 Get all orders of a user
	 * @param userid
	 */
	...
	/**
	 * 获取订单项目列表 Get a list of order items
	 * @param orderid
	 * @return
	 */
