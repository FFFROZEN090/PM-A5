...
public class PageUtil {
	...
	public static String getPageTool(HttpServletRequest request, long total, int page, int size){
		...
		if (page <= 1) {
			buf.append("<a class='btn btn-info' disabled >HomePage</a>\n");
		}else{
		...
		if (page >= pages) {
			buf.append("<a class='btn btn-info' disabled >End</a>\n");
		}else {
			buf.append("<a class='btn btn-info' href='").append(url).append("?page=").append(pages).append(queryString).append("'>End</a>\n");
		}
